...
class PacmanGUI:
    def __init__(self, root):
        ...
        # Create a button for the specific command to update the system
        self.update_button = tk.Button(self.frame, text="Update System", command=self.update_system)
        self.update_button.pack(side=tk.LEFT)

        # Create a button for updating the package database
        self.update_db_button = tk.Button(self.frame, text="Update Package Database", command=self.update_package_database)
        self.update_db_button.pack(side=tk.LEFT)

        # Create a button for yay commands
        self.yay_button = tk.Button(self.frame, text="Execute Yay Command", command=self.run_yay_command)
        self.yay_button.pack(side=tk.LEFT)
    ...
    def update_package_database(self):
        command = "pkexec pacman -Sy --noconfirm"
        self.output_area.delete(1.0, tk.END)  # Clear previous output
        self.output_area.insert(tk.END, f"Executing: {command}\n")
    ...
    def run_yay_command(self):
        command = self.command_entry.get()
        if not command.startswith('yay'):
            messagebox.showerror("Error", "Please use 'yay' commands only.")
            return
